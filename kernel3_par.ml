...
let rec sssp adjMatrix visited queue distance pool prev_root parent = 
	...
	(droot, root)::_ ->
		...
		match Lockfree.Hash.find adjMatrix root with
			...
			Some lst ->
				...
				Printf.printf "\n";*)
				visited.(root) <- 1;
				sssp adjMatrix visited queue distance pool root parent
...
let kernel3 () = 
	...
	true,_ -> 
		...
		let _ = sssp adjMatrix visited queue (Atomic.make distance) pool (-1) parent in
